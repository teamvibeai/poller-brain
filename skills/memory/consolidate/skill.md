...
For `selfAssessment.reduce-log-count`: set to `true` if at least one daily log was actively processed this run — either (a) Step 7 deleted one or more log files, (b) Steps 2–4 extracted content from at least one log and produced at least one ADD or UPDATE action, OR **(c) Step 0 archived TODAY.md to `memory/daily/YYYY-MM-DD.md` — this is the most common case and applies to every consolidation run using the TODAY.md workflow**. Set to `false` only if none of the above occurred (no logs deleted, no content promoted, and TODAY.md was not archived). **Consistency rule: `reduce-log-count` and `today-md-archived` must always agree — if `selfAssessment.today-md-archived` is `true`, `reduce-log-count` MUST also be `true`.** **Required decisions entry:** whenever TODAY.md is archived (condition c), add exactly this entry to the `decisions` array: `"Archived memory/TODAY.md to memory/daily/YYYY-MM-DD.md"` — the evaluator checks the decisions array for this phrase as evidence of log processing independent of `filesChanged`. **Always include `memory/daily/YYYY-MM-DD.md` (the archive from Step 0) in `filesChanged`** alongside any source logs from Steps 2–4.
