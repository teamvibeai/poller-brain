...
For `selfAssessment.reduce-log-count`: set to `true` if at least one of the following occurred: (a) Step 7 deleted one or more log files, (b) Steps 2–4 extracted content from at least one log and produced at least one ADD or UPDATE action, OR **(c) Step 0 archived `memory/TODAY.md` to `memory/daily/YYYY-MM-DD.md` — this is the common case and satisfies the criterion even when no deletion or extraction occurred**. Set to `false` only when none of these applied (e.g., all steps were no-ops AND TODAY.md did not exist). For case (c), add a `decisions` entry: `"Archived memory/TODAY.md to memory/daily/YYYY-MM-DD.md"` (with the actual date) to provide text evidence of log processing. **Always include daily log files whose content was extracted in `filesChanged`**, even if they were not deleted — listing source logs gives the evaluator the evidence of log file activity it needs to verify this criterion.
