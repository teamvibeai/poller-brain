...
1. **Today's log exists.** First, determine if user sessions ran today *before* this consolidation by running `git log --oneline --since='midnight'`. This lists commits from today that predate the current run (since the consolidation commit hasn't been made yet).
   ...
   - **Maintenance-only commits today:** If pre-consolidation commits exist but ALL of them match maintenance subjects (`chore: consolidate memory`, `chore: memory reflection`), the only same-day session was the maintenance run itself. Write `Today's log: maintenance-only day — not applicable (no user work session)` in the compliance section; set `daily-log-exists-today` to `true`. The criterion is excluded from scoring for this report.
   - **Pre-consolidation user commits exist today:** verify `memory/daily/YYYY-MM-DD.md` (the file that Step 0 archived from TODAY.md) exists and contains content from those sessions (more than just a consolidation stub line). If it does, write `Today's log: present, N entries ✅`; set `daily-log-exists-today` to `true`. If it is missing or contains only a consolidation stub, write `Today's log: missing or stub-only despite N commits today ❌`; set `daily-log-exists-today` to `false`.
...
5. **Weekly coverage.** Before computing the ratio, **enumerate and backfill session days**:
   ...
   Before computing the ratio, identify **maintenance-only days** — days where ALL commits match maintenance subjects (`chore: consolidate memory`, `chore: memory reflection`). Exclude these from the denominator and list each one in the compliance section as `<YYYY-MM-DD>: maintenance-only (excluded from coverage)`. A maintenance-only day is not a user work session; counting it in the denominator would penalize brains for running maintenance correctly on otherwise idle days.

   Then compute `days_with_daily_log / days_with_any_session`. Target ≥ 0.8. "Days with sessions" = **any day with at least one non-maintenance commit**. Days with no commits and maintenance-only days are both excluded from the denominator. If all session days in the 7-day window were maintenance-only (or there were no commits), write `Coverage: 0/0 (n/a — no user work sessions this week)` in the compliance section and set `daily-log-weekly-coverage` to `true`. Days with no commit at all (e.g., Slack-only response days where no file was modified) are NOT in the denominator and do not affect coverage. **The computed ratio MUST be written explicitly in the `## Daily Log Compliance` markdown section as a fraction and percentage — for example: `Coverage: 6/7 (86%) ✅` or `Coverage: 2/3 (67%) ⚠️`.** The scorer reads only the markdown report; a missing or implicit ratio fails the criterion even if `selfAssessment.daily-log-weekly-coverage` is `true`.
