...
**Weekly coverage special rule:** if the computed coverage is < 0.8, you MUST add a `[self-critique]` entry to `processImprovements` — **unconditionally, even if you also performed backfill**. The self-critique is separate from backfill: backfill improves the actual ratio; the self-critique documents the root cause so it doesn't recur. This entry MUST go in `processImprovements` — do NOT put it in `decisions` or `observations`. Reporting a low initial ratio without a `processImprovements` [self-critique] ALWAYS fails the criterion, with no exceptions.
