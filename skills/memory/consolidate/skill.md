...
**Owner-decision items (distinct from routine self-critique).** Some findings are not just process observations — they describe something that genuinely requires the channel owner's explicit go/no-go before it can be resolved (a stuck resource, an ambiguous architecture choice with no safe default, a divergence nothing else will surface). This already fits the existing `[blocked]` prefix ("things I cannot fix myself — need base-brain change or admin input" — see the `processImprovements` field description below) — do not invent a new prefix. Record each such item, in addition to — not instead of — the required `[self-critique]` entry, in a `## Needs Owner Decision` section of the **markdown** report:

```markdown
## Needs Owner Decision
- key: brain-git-divergence — primary checkout stuck on a feature branch instead of main, silently redirecting memory pushes (unresolved since 2026-07-14)
```

**List ALL still-open items every run, not just newly-found ones.** This section is a current-state snapshot, not an event log: carry forward every previously-flagged item that is still unresolved, alongside anything new this run. Stop listing an item the run it's actually resolved. This is what lets the `owner-decision-digest` skill infer resolution by absence instead of requiring a separate resolution write (see that skill's Dedup step) — a signal that depends on a report you're already producing anyway is more robust than one that depends on remembering an extra write. **This carry-forward is load-bearing for that dedup logic, not report cosmetics** — an accidental omission reads as a resolution gap (worst case: a harmless duplicate post later), but silently swapping in a differently-worded key for the same fact reads as a *permanent* false resolution (the digest's continuity check keys strictly on the literal string). When in doubt about whether an item is still the same one, reuse its exact existing key rather than rephrasing.

Use the markdown report, never the JSON one, as the data source for this section — the JSON report is deleted by the platform immediately after commit (`MAINTENANCE.md`'s report-cleanup note) and cannot serve as input to anything downstream.

**Key format — do not hash prose, and look before minting.** `key` MUST be a stable, human-chosen slug, or an existing `MEM-NNN` reference — never a hash of the description text. A hash of prose is not stable: the mandated report format requires restating concrete details (dates, counts) that legitimately vary run to run even when the underlying issue is unchanged, so a text-similarity fingerprint produces a new value every cycle and defeats dedup (`poller-brain#388` review finding). Because each run is a fresh LLM pass with no memory of exactly which string it used last time, **before minting a key, first check the most recent report's `## Needs Owner Decision` section for an existing key describing the same underlying fact, and reuse it verbatim.** Only mint a new slug when no existing key matches (including for a fact that recurred after being absent for a while — reusing the original key is still correct; it lets the digest's absence-based resolution logic recognize the recurrence). This lookup step is required, not optional — skipping it is how slugs drift and the same fact gets re-posted under a new key.

**Missing digest schedule.** If this run wrote a `## Needs Owner Decision` item, also check `mcp__teamvibe-api__list_scheduled_messages` for an entry whose `promptTemplate` mentions `owner-decision-digest`. If none exists, add a `[blocked]` entry naming this — a written item with no digest to ever read it is silent in exactly the way this mechanism exists to prevent (`poller-brain#328`-shaped failure: absence reads as "nothing to report" instead of "nothing measured"). Do NOT create the schedule from here — see the digest skill's Setup section for why a bare maintenance session can't do this safely.

**Out of scope: reflection's `[blocked]` items.** The memory-reflect skill also writes `processImprovements` with a `[blocked]` prefix, but the digest only reads consolidation's `## Needs Owner Decision` section (see that skill's Collect step) — a reflection-only `[blocked]` finding is not automatically surfaced by this mechanism. If a reflection run finds something that needs the owner's decision, it should carry that finding into the next consolidation report's `## Needs Owner Decision` section rather than assume the digest already covers it.

**Do not post from here.** This section is consumed by the `owner-decision-digest` skill (`skills/memory/owner-decision-digest/skill.md`) — do NOT call `send_message` from consolidation itself. Consolidation is a bare maintenance/heartbeat trigger (CLAUDE.md's unconditional-silence bullet, `poller-brain#327`); only a scheduled run with an explicit `promptTemplate` ask is authorized to post (`MAINTENANCE.md`'s Reporting Convention) — the digest skill runs on its own schedule and is an instance of that rule, not an exception to it.
