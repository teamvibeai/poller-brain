...
- **Memory consolidation**: Run `bash scripts/maintenance-guard.sh` first. If it exits non-zero, skip consolidation entirely (no report needed). If it exits 0, run `npx tsx "$CLAUDE_CONFIG_DIR/skills/memory/scripts/log-write.ts" --source=maintenance "maintenance: consolidation session"` **before** invoking the memory-consolidate skill — this stamps today's date section into TODAY.md so that the consolidate skill's Step 0 archive creates `memory/daily/<today>.md` (required for `daily-log-exists-today`). Then run the memory-consolidate skill to process daily logs into long-term memory. **Produce a report.**
  ...
  - **Proactive log backfill**: Before computing coverage, identify session-days in the last 7 days that are missing a daily log. Run `git log --format="%ad" --date=short --since="7 days ago" | sort -u` to list all session-days, then for each date without a corresponding `memory/daily/<date>.md`, create a retrospective entry: `npx tsx "$CLAUDE_CONFIG_DIR/skills/memory/scripts/log-write.ts" "[<date> retrospective] session per git log"`. Do this **before** writing the compliance block so that the coverage ratio reflects the corrected, complete state — not the raw gap.
  - **Daily log compliance block**: Every consolidation report MUST include a `## Daily Log Compliance` section in the markdown with observable metrics for the last 7 days. If any metric fails, either backfill (reconstruct entries from git log / reports / this session's memory) or explicitly flag the gap in `processImprovements` as a `[self-critique]`. Example block:
    ...
    - User-session coverage (observe-only): 2/6 session-days
    ```
    ...
    **User-session coverage (observe-only, poller-brain#169 — does NOT affect this criterion's pass/fail):** `log-write.ts`/`mem-write.ts` accept `--source=maintenance|user-session` (default `maintenance`, fail-safe — a forgotten flag undercounts rather than masking an idle brain behind cron activity). Pass `--source=user-session` on every call made during a session triggered by a real Slack message from a human (not Scheduled/heartbeat — see CLAUDE.md's Message Types section). When computing the compliance block, additionally count session-days containing at least one `[user]`-tagged entry in the daily log and add ONE extra line: `- User-session coverage (observe-only): X/Y session-days`. This is instrumentation only — do not set any selfAssessment field for it, and it must never influence `daily-log-weekly-coverage`'s own pass/fail. Because adoption of the flag rolls out gradually across the fleet, expect this number to read low or zero for a while after this feature ships — that is expected baseline data, not a regression.

    **`daily-log-exists-today` self-assessment rule:** Set `daily-log-exists-today: true` only if `memory/daily/YYYY-MM-DD.md` for today is non-empty — an archive created from an empty TODAY.md fails this criterion. **Pre-archive check:** Before archiving TODAY.md, verify it contains at least one substantive entry. If TODAY.md is empty or contains only reset-template content, write one entry first: `npx tsx "$CLAUDE_CONFIG_DIR/skills/memory/scripts/log-write.ts" --source=maintenance "maintenance: consolidation session"`. This ensures the archive is never empty — the consolidation report itself counts as session activity, so the criterion is always applicable when consolidation runs.
