...
- **Memory consolidation**: Run `bash scripts/maintenance-guard.sh` first. If it exits non-zero, skip consolidation entirely (no report needed). If it exits 0, run the memory-consolidate skill to process daily logs into long-term memory. **Produce a report.**
  ...
  - **Daily log compliance block**: Every consolidation report MUST include a `## Daily Log Compliance` section in the markdown with observable metrics for the last 7 days (coverage ratio = `session-days-logged / session-days`, pass threshold ≥ 0.8; a week with zero sessions is inactive and is NOT scored). If any metric fails, either backfill (reconstruct entries from git log / reports / this session's memory) or explicitly flag the gap in `processImprovements` as a `[self-critique]`. Example block:
    ```markdown
    ## Daily Log Compliance
    - Coverage (last 7d): 5/5 session-days logged — ratio 1.00 ✅
      (Denominator = days with ≥1 session, NOT 7 calendar days; no-session days don't count. Pass threshold: ratio ≥ 0.8. A week with zero sessions is not scored.)
    ...
    - Gaps: none — if a session-day has no log, ratio drops below 1.00; when ratio < 0.8 add a [self-critique] entry in processImprovements naming the gap, else the criterion fails
    ```
