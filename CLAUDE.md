...
The same isolation rule applies to **PR rationales**, not just runtime features. If a base-brain PR's rationale contains a claim like *"only N brains use X"*, *"no brains do Y"*, or *"this is safe for all brains"*, that claim must be backed by one of:
...
Reviewers are empowered to block a base-brain PR on missing or GitHub-enumeration-based evidence.
