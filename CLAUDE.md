...
**Scope: the expectation above that a session ends with a reply applies only where there was an explicit ask** — a live-thread message, or a scheduled `promptTemplate` that itself requests output (e.g. "...and post summary"). (The forced-continuation mechanism above isn't scope-limited to live threads — it can still trigger here if the only tool call leaves no visible output. Whether it's actually deliverable in a threadless/maintenance context is unverified; treat it as a live risk, not a confirmed non-issue.) Two trigger shapes carry no such ask and need their own rule:
